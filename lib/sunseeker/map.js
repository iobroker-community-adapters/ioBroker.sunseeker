...
module.exports = {
        ...
        try {
            ...
            if (mapChanged || !meta.mapJson) {
                ...
                if (backup.json && backup.json.data) {
                    ...
                    this.log.debug(`fetchMap ${sn}: Backup-Map emitted`);
                }
            }
            try {
                this.log.debug(
                ...
                if (dataUrl) {
                    ...
                    this.log.debug(`fetchMap ${sn}: Livemap emitted (${dataUrl.length} Bytes data URL)`);
                } else {
                    this.log.debug(`fetchMap ${sn}: Livemap not rendered (no geometry)`);
                }
            } catch (err) {
    ...
    async fetchMapJson(sn, kind, url) {
        if (!url) {
            this.log.debug(`fetchMapJson ${sn}/${kind}: No URL`);
            return null;
        }
        ...
        if (res.status !== 200 || res.data == null) {
            this.log.debug(`fetchMapJson ${sn}/${kind}: HTTP ${res.status} empty`);
            return null;
        }
        let parsed;
        let payload;
        ...
        this.log.debug(`fetchMapJson ${sn}/${kind}: ${payload.length} Bytes emitted`);
        return parsed;
    },
    ...
    async fetchMapImage(sn, kind, url) {
        if (!url) {
            this.log.debug(`fetchMapImage ${sn}/${kind}: No URL!`);
            return;
        }
        ...
        if (res.status !== 200 || !res.data) {
            this.log.debug(`fetchMapImage ${sn}/${kind}: HTTP ${res.status} empty!`);
            return;
        }
        ...
        this.log.debug(`fetchMapImage ${sn}/${kind}: ${ct}, ${buf.length} Bytes emitted`);
    },
