...
module.exports = {
    ...
    onMqttMessage(topic, payload) {
        ...
        const sn = data.deviceSn.replace(this.iob.FORBIDDEN_CHARS, "_");
        ...
        if (id === "getSupportCustomPatternSet") {
            if (typeof statusData.support_custom_pattern === "object" && statusData.support_custom_pattern !== null) {
                const new_pattern = {};
                for (const att of statusData.support_custom_pattern) {
                    new_pattern[att] = att;
                }
    ...
    async setDeviceProperty(sn, body) {
        ...
        this.iob.log.debug(`setDeviceProperty: ${JSON.stringify(data)}`);
        try {
            await this.request(
                "POST",
