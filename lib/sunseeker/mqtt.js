...
module.exports = {
    ...
    async startMqttNew() {
        ...
        try {
            ...
            this.log.error(`Setting MQTT password failed: ${err.message}`);
            this.emit("error", err);
            return;
        }
        if (this.unloading) {
    ...
    connectMqtt() {
        ...
        this.log.info(`MQTT connection ${url} username=${username}`);
    ...
    scheduleMqttRetry() {
        ...
        this._mqttRetryTimer = this.iobTimer.setTimeout(() => {
    ...
    startMqttOld() {
        ...
        const userId = this.session.user_id;
        this.log.info(`Connect MQTT (Old) ${url}`);
        const client = mqtt.connect(url, {
        ...
        client.on("connect", () => {
            this.log.info("MQTT (Old) Connected");
            this.emit("mqttConnect");
        ...
        client.on("close", () => {
            ...
            this.log.debug("MQTT (Old) closed");
            this.emit("mqttDisconnect");
            if (!this.unloading && !this._mqttOldRetryTimer) {
                this._mqttOldRetryTimer = this.iobTimer.setTimeout(() => {
    ...
    onMqttMessage(topic, payload) {
        this.log.debug(`MQTT: ${topic} - ${payload.toString("utf8")}`);
        let data;
        try {
        ...
        if (!sn || !this.devicesRaw[sn]) {
            this.log.debug(`MQTT for an Unknown Device: ${topic} ${JSON.stringify(data).slice(0, 120)}`);
            return;
        }
        ...
        if (meta._refreshTimer) {
            this.iobTimer.clearTimeout(meta._refreshTimer);
        }
        meta._refreshTimer = this.iobTimer.setTimeout(() => {
            meta._refreshTimer = null;
            this.updateDevice(sn).catch(err => this.log.debug(`Refresh after MQTT (${sn}): ${err.message}`));
    ...
    async fetchInitialProperties() {
        ...
        this.log.debug(`fetchInitialProperties: ${sns.length} Device(s)`);
        for (const sn of sns) {
            const meta = this.deviceMeta[sn];
